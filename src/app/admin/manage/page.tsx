...
interface Consultation {
    ...
    message: string;
    status: 'pending' | 'followed_up' | 'closed' | string;
    submittedAt: string;
}
...
function ConsultationManagement() {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            // สมมติว่ามี API DELETE endpoint
    ...
    const handleUpdateStatus = async (consultation: Consultation, newStatus: string) => {
        ...
        try {
            // สมมติว่ามี API PUT/PATCH endpoint
...
export default function AdminDashboard() {
    const [activeTab, setActiveTab] = useState<'centers' | 'consultations'>('centers');
